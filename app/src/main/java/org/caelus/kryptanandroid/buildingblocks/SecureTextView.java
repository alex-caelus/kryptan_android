...
public class SecureTextView extends TextView implements OnLayoutChangeListener
{
	...
	public int getRealInputType()
	{
		if (mRealInputType == -1)
		{
			setRealInputType(this.getInputType());
		}
		return mRealInputType;
	}

	public void setRealInputType(int type)
	{
		mRealInputType = type;
	}
	...
	public void onDraw(Canvas canvas)
	{
		...
		if (mText != null && mText.GetLength() != 0)
		{
			...
			for (int currentLine = 0; currentLine < lineCount; currentLine++)
			{
				...
				{
					if ((this.getRealInputType() & InputType.TYPE_MASK_VARIATION) == InputType.TYPE_TEXT_VARIATION_PASSWORD)
						mTmpArr[i] = '•';
					else
